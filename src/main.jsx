...
function Dashboard({ onLogout }) {
  ...
  const [posts, setPosts] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('food-posts') || '[]');
    } catch {
      return [];
    }
  });
  const [reports, setReports] = useState(() => { try { const saved = JSON.parse(localStorage.getItem('anganwadi-reports') || 'null'); if (Array.isArray(saved)) return saved; const legacy = JSON.parse(localStorage.getItem('anganwadi-report') || 'null'); return legacy?.locked ? [legacy] : []; } catch { return []; } });
  const [students, setStudents] = useState(() => { try { return JSON.parse(localStorage.getItem('anganwadi-students') || '[]'); } catch { return []; } });
  ...
  useEffect(() => localStorage.setItem('anganwadi-reports', JSON.stringify(reports)), [reports]);
  useEffect(() => localStorage.setItem('anganwadi-students', JSON.stringify(students)), [students]);
  ...
  const confirmShare = () => { setAnalyzing(true); window.setTimeout(async () => { const now = new Date(); const stampedImage = await makeStampedPhoto(); const next = { id: photoId || `FS-${Math.floor(1000 + Math.random() * 8999)}`, food: snackName, category: snackName, description: `${snackName} photo ready to share.`, items: ['Visible ingredients pending'], calories: 'Estimate unavailable', status: 'Unknown', user: 'Aniket', time: `${formatDate(now)}, ${formatTime(now)}`, createdAt: now.toISOString(), image: stampedImage, capturedAt: capturedAt?.toISOString() || now.toISOString(), location, note }; setPosts(current => [next, ...current]); setAnalyzing(false); setShowCapture(false); setImage(null); setActive('Overview'); await shareToWhatsApp(next); }, 2100); };
  ...
  const directWhatsAppShare = async () => { const now = new Date(); window.open(whatsappGroupLink, '_blank', 'noopener,noreferrer'); const stampedImage = await makeStampedPhoto(); const next = { id: photoId || `FS-${Math.floor(1000 + Math.random() * 8999)}`, food: snackName, category: snackName, description: `${snackName} photo captured from Anganwadi Software.`, items: ['Details will be added later'], calories: 'Not estimated', status: 'Unknown', user: 'Aniket', time: `${formatDate(now)}, ${formatTime(now)}`, createdAt: now.toISOString(), image: stampedImage, capturedAt: capturedAt?.toISOString() || now.toISOString(), location, note }; setPosts(current => [next, ...current]); if (stampedImage?.startsWith('data:')) { const link = document.createElement('a'); link.href = stampedImage; link.download = `${next.id}.jpg`; document.body.appendChild(link); link.click(); link.remove(); } setShowCapture(false); setImage(null); setCameraReady(false); };
